...
class StreamInfo:
    ...
    lsl_StreamInfo: InitVar[pylsl.StreamInfo]
    """pylsl stream info object."""

    def __post_init__(self, lsl_StreamInfo: pylsl.StreamInfo):
        """Actually init members from stream info."""
        self.name = lsl_StreamInfo.name()
        self.sample_rate = lsl_StreamInfo.nominal_srate()
        self.channel_count = lsl_StreamInfo.channel_count()
...
class LSLInput(Input):
    ...
    def __init__(
        ...
        self._StreamInfo: Optional[pylsl.StreamInfo] = None
    ...
    def get_info(self) -> StreamInfo:
        ...
        results = pylsl.resolve_byprop("name", self._stream_name)
    ...
    def connect(self):
        ...
        self._StreamInfo = self._get_stream()
        ...
        self._inlet = LSLInput._create_inlet(self._StreamInfo)
        self._logger.info(f"Connected to LSL input stream: '{self._stream_name}'")

        bufsize = (
            200 * math.ceil(self._StreamInfo.nominal_srate()),
            self._StreamInfo.channel_count(),
        )
        self.buffer = np.empty(
            bufsize, dtype=LSL_DTYPES[self._StreamInfo.channel_format()]
        )
    ...
    def _get_stream(self) -> pylsl.StreamInfo:
        ...
        """
        StreamInfos = pylsl.resolve_byprop(
    ...
    def _get_available_streams(self) -> List[StreamInfo]:
        StreamInfos = pylsl.resolve_streams(wait_time=self._resolve_streams_wait_time)
        return [StreamInfo(lsl_StreamInfo) for lsl_StreamInfo in StreamInfos]

    @staticmethod
    def _create_inlet(StreamInfo: pylsl.StreamInfo) -> pylsl.StreamInlet:
