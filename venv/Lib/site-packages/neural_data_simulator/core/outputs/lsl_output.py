...
class LSLOutputDevice(Output):
    ...
    def _get_open_stream_names() -> List[str]:
        ...
        return [lsl_StreamInfo.name() for lsl_StreamInfo in StreamInfos]

    def connect(self):
        """Connect to the LSL stream."""
        self.logger.info("Initializing LSL output stream...")

        self._StreamInfo = LSLOutputDevice._get_info_from_config(self._stream_config)

        outlet_buffer_time_in_s = 1
        self._outlet = pylsl.StreamOutlet(
            self._StreamInfo, max_buffered=outlet_buffer_time_in_s
        )
        self.logger.info(f"Created LSL output stream: '{self._StreamInfo.name()}'")
    ...
    @staticmethod
    def _build_xml_from_dict(node, dict_info: dict):
